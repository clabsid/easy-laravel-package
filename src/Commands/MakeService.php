...
class MakeService extends Command
{
    ...
    public function createService(string $className)
    {
        ...
        $serviceName = $nameOfService . config("easy-laravel-package.service_suffix");

        $namespace = $this->getNameSpace($className);
        $stubProperties = [
            "{namespace}" => $namespace,
            "{serviceName}" => $serviceName,
            "{serviceInterface}" => $nameOfService. config("easy-laravel-package.service_interface_suffix"),
    ...
    {
        $nameOfService = $this->getServiceName($className);
        $serviceName = $nameOfService . config("easy-laravel-package.service_interface_suffix");
    ...
    private function getServicePath($className, $servicename)
    {
        return $this->appPath() . "/" .
            ...
            "/$className". "/$servicename" . config("easy-laravel-package.service_suffix") .".php";
    }
    ...
    {
        return config("easy-laravel-package.repository_namespace") . "\Eloquent";
    }
    ...
    {
        return $className . config("easy-laravel-package.repository_interface_suffix");
    }
    ...
    {
        $this->ensureDirectoryExists(config("easy-laravel-package.service_directory"));
    }
    ...
    private function getNameSpace($className):string {
        ...
        if (count($explode) > 1) {
            ...
            return config("easy-laravel-package.service_namespace").$namespace."\\".end($explode);
        } else {
            return config("easy-laravel-package.service_namespace")."\\".$className;
        }
    }
