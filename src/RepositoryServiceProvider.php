...
use EasyLaravelPackage\helpers\Search;
use SplFileInfo;
...
class RepositoryServiceProvider extends ServiceProvider
{
    ...
    private function bindAllRepositories()
    {
        ...
        foreach ($repositoryInterfaces as $key => $repositoryInterface) {
            $repositoryInterfaceClass =  config("easy-laravel-package.repository_namespace") . "\\"
                . $repositoryInterface . "\\"
                . $repositoryInterface
                . config("easy-laravel-package.repository_interface_suffix");

            $repositoryImplementClass = config("easy-laravel-package.repository_namespace") . "\\"
                . $repositoryInterface . "\\"
                . $repositoryInterface
                . config("easy-laravel-package.repository_suffix");
    ...
    private function bindAllServices()
    {
        ...
        foreach ($servicePath as $serviceName) {
            ...
            $serviceInterfaceClass =  config("easy-laravel-package.service_namespace") . "\\"
                . $pathService . "\\"
                . $className
                . config("easy-laravel-package.service_interface_suffix");

            $serviceImplementClass = config("easy-laravel-package.service_namespace") . "\\"
                . $pathService . "\\"
                . $className
                . config("easy-laravel-package.service_suffix");
    ...
    {
        $folders = [];
        if (file_exists($this->app->basePath() . "/" . config("easy-laravel-package.repository_directory"))) {
    ...
    {
        return config("easy-laravel-package.repository_namespace") .
            "\\" . $className;
    }
    ...
    {
        return $className . config("easy-laravel-package.repository_suffix");
    }
    ...
    private function getServicePath()
    {
        $root = $this->app->basePath() .
            "/" . config("easy-laravel-package.service_directory");
        $servicePath = [];
    ...
    {
        $path = config_path("easy-laravel-package.php");
